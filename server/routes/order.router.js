...
router.get("/", async (req, res) => {
  try {
    const order = await Order.find().populate(['CustomerID','ShipperID', 'EmployeeID', 'Products.ProductID']);
    res.status(201).json({
      success: true,
      data: order,
    });
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const order = await Order.findOne({ _id: id }).populate(['CustomerID','ShipperID', 'EmployeeID', 'Products.ProductID']);
    res.json({
      success: true,
      data: order,
    });
